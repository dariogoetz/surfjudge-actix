...
impl HeatAdvancement {
    async fn expand(mut self, db: &Pool) -> Self {
        self.to_heat = Heat::find_by_id(&db, self.to_heat_id as u32, false)
            .await
            .unwrap_or(None);
        self.from_heat = Heat::find_by_id(&db, self.from_heat_id as u32, false)
            .await
            .unwrap_or(None);
        self
    }
    ...
    pub async fn find_by_to_heat_id(
        db: &Pool,
        value: u32,
        expand: bool,
    ) -> anyhow::Result<Vec<Self>> {
    ...
    pub async fn find_by_from_heat_id(
        db: &Pool,
        value: u32,
        expand: bool,
    ) -> anyhow::Result<Vec<Self>> {

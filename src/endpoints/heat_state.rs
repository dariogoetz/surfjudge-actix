...
pub async fn get_by_heat_id(web::Path(heat_id): web::Path<u32>, db: web::Data<Pool>) -> Result<web::Json<ResultHeatState>> {
    let result = HeatState::find_by_heat_id(db.get_ref(), heat_id).await.map_err(|e| {
